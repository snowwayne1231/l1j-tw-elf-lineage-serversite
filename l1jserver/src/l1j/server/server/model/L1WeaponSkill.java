...
public class L1WeaponSkill {
	...
	public static double getWeaponSkillDamage(L1PcInstance pc, L1Character cha,
		...
		int chance = Random.nextInt(100 + pc.getWeapon().getEnchantLevel()) + 1;
	...
	public static double getBaphometStaffDamage(L1PcInstance pc, L1Character cha) {
		...
		int chance = Random.nextInt(100 + elevel) + 1;
	...
	public static double getAreaSkillWeaponDamage(L1PcInstance pc,
		...
		int attr = 0;
		int chance = Random.nextInt(100+probability) + 1;
		...
		if (probability >= chance) {
			...
			if (weaponId == 263 || weaponId == 290) { // 冰凍長矛兵
				area = 3;
				damageRate = 1.4D;
				effectTargetId = cha.getId();
				effectId = 1804;
				areaBase = cha;
			} else if (weaponId == 260) { // 狂風之息
				area = 4;
				damageRate = 1.5D;
