		...
		if (pc.getCurrentHp() > 0) {
			...
			if ((itemId == 40077) || (itemId == L1ItemId.SCROLL_OF_ENCHANT_WEAPON) || (itemId == L1ItemId.SCROLL_OF_ENCHANT_QUEST_WEAPON)
				...
				if (l1iteminstance.getItem().getType() == 0) { // アロー
					...
					pc.addBaseMaxHp((short)(-(pc.getBaseMaxHp() / 2)));
					pc.addBaseMaxMp((short)(-(pc.getBaseMaxMp() / 2)));
					pc.setExp(1);
					pc.resetLevel();
					pc.setBonusStats(0);
					...
					if ((pc.getBaseStr() < 50) && (pc.getElixirStats() < 5)) {
					...
					if ((pc.getBaseCon() < 50) && (pc.getElixirStats() < 5)) {
					...
					if ((pc.getBaseDex() < 50) && (pc.getElixirStats() < 5)) {
					...
					if ((pc.getBaseInt() < 50) && (pc.getElixirStats() < 5)) {
					...
					if ((pc.getBaseWis() < 50) && (pc.getElixirStats() < 5)) {
					...
					if ((pc.getBaseCha() < 50) && (pc.getElixirStats() < 5)) {
				...
				// 治癒藥水、濃縮體力恢復劑、象牙塔治癒藥水
					...
					if (bookm != null) { // ブックマークを取得出来たらテレポート
						...
						if (pc.getMap().isTeleportable() || pc.isGm()) {
							L1Location newLocation = pc.getLocation().randomLocation(90, true);
